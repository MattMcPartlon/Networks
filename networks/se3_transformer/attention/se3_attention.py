...
from networks.common.helpers.torch_utils import batched_index_select, safe_norm, safe_cat
from networks.common.utils import exists
from networks.config.se3_attention_config import SE3AttentionConfig
from networks.common.repr.fiber import Fiber, to_order, cast_fiber
from networks.common.equivariant.units.linear import VNLinear
from networks.common.equivariant.units.fiber_units import FiberLinear
from networks.common.invariant.distance_conv import PairwiseDistanceConv
from networks.common.constants import DIST_SCALE
from typing import Tuple, Dict, Optional
from networks.se3_transformer.attention.utils import (
...
class SE3Attention(nn.Module):
    def __init__(
        ...
        :param config: SE#Attention Config
        """
        ...
        self.dist_conv = None
        if config.use_dist_conv:
            self.dist_conv = PairwiseDistanceConv(
                dim_in=(fiber_in[1], fiber_in[1]),
                n_filters=config.num_dist_conv_filters,
                pairwise=config.pairwise_dist_conv,
    ...
    def augment_edges(
        ...
        if config.use_dist_conv:
            hidden_coords = compute_hidden_coords(initial_coords=neighbor_info.coords, coord_feats=features["1"])
            nbr_coords = batched_index_select(hidden_coords, neighbor_info.indices, dim=1)
            rel_posns = rearrange(hidden_coords, "b n N h c -> b n () h () c") - \
                        rearrange(nbr_coords, "b n N h c -> b n N () h c")  # noqa
            rel_dists = safe_norm(rel_posns * DIST_SCALE, dim=-1)
            kds = self.dist_conv(edges=edges, dists=rel_dists)
            edge_feats = safe_cat(edge_feats, kds, dim=-1)
